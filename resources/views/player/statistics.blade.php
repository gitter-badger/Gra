...
<div class="text-center">
	...
	<?php $damage = $player->getDamage(); ?>
	{!! BootForm::staticInput('<strong>' . trans('statistic.damage') . '</strong>')
		->value($damage[0] . ' - ' . $damage[1]) !!}

	{!! BootForm::staticInput('<strong>' . trans('statistic.defense') . '</strong>')
		->value($player->getDefense()) !!}

	{!! BootForm::staticInput('<strong>' . trans('statistic.speed') . '</strong>')
		->value($player->getSpeed()) !!}

	{!! BootForm::staticInput('<strong>' . trans('statistic.critChance') . '</strong>')
		->value($player->GetCritChance() . '%') !!}
